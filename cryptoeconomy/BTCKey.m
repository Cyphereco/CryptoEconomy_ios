...
//    printf("signature:%s, length:%lu", signatureBytes, (unsigned long)signature.length);
...
////  cryptoeconomy
////
////  Created by JJ Cherng on 2020/5/3.
...
////                  0x1D = second key with even y, 0x1E = second key with odd y,
////                  add 0x04 for compressed keys.
//static NSData* processSignedMessage(NSMutableData* publicKey, NSData* hash, NSData* signature) {
//////    CHECK_IF_CLEARED;
//    NSMutableData* sigdata = [NSMutableData dataWithLength:65];
//    unsigned char* sigbytes = sigdata.mutableBytes;
//    const unsigned char* hashbytes = hash.bytes;
...
//
//    unsigned char *p64 = (sigbytes + 1); // first byte is reserved for header.
//    ECDSA_SIG *sig = d2i_ECDSA_SIG(NULL, (const unsigned char**) &sigbytes, 65);
////
//////    ECDSA_SIG *sig = ECDSA_do_sign(hashbytes, hashlength, _key);
//////    if (sig==NULL) {
//////        return nil;
